...
export class GameManager {
  ...
  createSession({ gameId = 'claim-the-land', pin, requireApproval = config.requireApprovalDefault }) {
  ...
  _matchBeginPayload(game, match, side, opponentName) {
    ...
    return {
      ...
      rivalMeters: { ...state.sides[otherSide(game, side)].meters },
    };
  }
  ...
  _afterMove(session, game, match, side, res) {
    ...
    // chapter using its nation's real (historically right) strategy.
    if (match.solo && res.chapterDone) {
      const aiSide = otherSide(game, side);
      emits.push(...this._driveAIChapter(game, match, aiSide));
    }
  ...
  _completeMatch(session, game, match) {
    ...
    for (const [side, sid] of players) {
      ...
      const rival = report.perSide[rivalSide];
      ...
      emits.push(toStudent(sid, 'match:end', {
        ...
        you: mine, // claimScore, meters, regionsHeld, collapsed, accuracy, ending, debrief, accuracyNote?
        rival: {
          side: rivalSide,
    ...
    if (state.status === 'completed') {
      ...
      return {
        ...
        matchEnd: {
          ...
          rival: {
            side: rivalSide,
            claimScore: report.perSide[rivalSide].claimScore,
            regionsHeld: report.perSide[rivalSide].regionsHeld,
            collapsed: report.perSide[rivalSide].collapsed,
            isAI: report.perSide[rivalSide].isAI,
          },
        },
      };
    }
