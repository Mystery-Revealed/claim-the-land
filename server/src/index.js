...
app.get('/healthz', (_req, res) =>
  res.json({ ok: true, service: 'claim-the-land', liveSessions: manager.registry.sessions.size }));
...
server.listen(config.port, () => {
  console.log(`[claim-the-land] listening on :${config.port} — sessions live in memory only`);
});
